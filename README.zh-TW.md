...
| `http_methods_audit` | 回報伺服器允許的 HTTP 方法並分級風險(TRACE/PUT/DELETE)|
...
`http_headers_audit`、`cookie_audit`、`cors_check`、`tech_detect`、`http_methods_audit`、`well_known_audit`、
`ip_info` 或 `port_scan`。
...
回報伺服器允許哪些 HTTP 請求方法並分級風險。啟用的寫入/診斷方法會擴大攻擊面:
**TRACE** 會造成 Cross-Site Tracing(XST);**PUT** / **DELETE** 在存取控制
不足時可能讓人上傳或刪除檔案。**設計上安全** —— 絕不送出會變更狀態的請求:只
主動探測 OPTIONS、HEAD、TRACE(TRACE 僅回放),而 PUT / DELETE / PATCH /
CONNECT 只從 OPTIONS 的 `Allow` 標頭判讀為「已公告」,絕不實際送出。

回傳 `grade`、`score`、`allow_header`、`advertised_methods`、`trace_enabled`、
`dangerous_methods`,以及 `findings` 清單(每筆含方法、severity 與建議)。
